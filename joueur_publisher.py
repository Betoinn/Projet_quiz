...
def build_client(pseudo, code):
    ...
    pub = paho.Client(
        ...
        client_id=f"joueur-pub-{pseudo}-{code}",  # ID unique par joueur et partie
        protocol=paho.MQTTv5
    )
    # LWT 
    pub.will_set(state.topic(code, f"presence/{pseudo}"), "offline", qos=1, retain=True)
...
def on_connect(client, userdata, flags, rc, pseudo, code, properties=None):
    ...
    client.publish(state.topic(code, f"presence/{pseudo}"), "pret", qos=1, retain=True)
...
def publier_deconnexion(client, pseudo, code):
    client.publish(state.topic(code, f"presence/{pseudo}"), "offline", qos=1, retain=True)
